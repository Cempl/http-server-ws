...
/*******************************************************************************/
int SendRecv::my_send(SOCKET& client_socket, string& message)
{
	int result = 0;
...
/*******************************************************************************/
int SendRecv::my_recv(SOCKET& client_socket, string& message)
{
	char buffer[1024] = {0};

	int result = recv(client_socket, buffer, 1024, 0);
...
int SendRecv::Thread_recv(SOCKET client_socket)
{
	...
	while (true)
	{
		int result = my_recv(client_socket, message); // Принимаем данные от клиента

		result = recv_data(message);
